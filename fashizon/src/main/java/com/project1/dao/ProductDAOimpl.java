...
private void saveProductImage(Product prd){
	...
	try{				    	
		if (prd.getImagefile() != null ) {	
           	...
           	Path path=Paths.get("C://bean wrkspace//fashizon//src//main//webapp//resources//images//products//"+prd.getProductid()+".jpg");	
...
public void deleteProduct(String prd) {
	Session ses = sessionFactory.openSession();
	Transaction tr=ses.beginTransaction();
	Product temp=(Product)ses.get(Product.class,prd);
	ses.delete(temp);
	tr.commit();
	ses.close();	
}
