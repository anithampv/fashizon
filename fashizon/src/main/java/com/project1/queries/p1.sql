-- jdbc:h2:tcp://localhost/~/DT15Toys
...
-- to display best price products by excluding out of stock products
select * from vw_xmap_ps x1 where productsupplierprice=
( select min(productsupplierprice) from vw_xmap_ps  x2 where x1.productid=x2.productid and productsupplierstock>=8)
